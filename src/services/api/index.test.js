...
describe('parseSettings', () => {
  it('has method get by default', () => {
    expect(parseSettings().method)
      .toBe('get');
  });

  it('has normal body', () => {
    expect(parseSettings({ body: 'foo' }).body)
      .toBe('foo');
  });
  ...
  it('has passed method', () => {
    expect(parseSettings({ method: 'post' }).method)
      .toBe('post');
  });
...
describe('parseEndpoint', () => {
  it('appends endpoint to apiUrl', () => {
    expect(parseEndpoint('/foo'))
      .toBe('https://api.foo.com/foo');
  });

  it('parses params', () => {
    expect(parseEndpoint('/foo', { bar: 'baz' }))
      .toBe('https://api.foo.com/foo?bar=baz');
  });

  it('parses url other than apiUrl', () => {
    expect(parseEndpoint('https://foo.bar/baz'))
...
describe('api', () => {
  ...
  describe('create', () => {
    ...
    it('has settings', () => {
      expect(api.create({ foo: 'bar' }).settings)
        .toEqual({ foo: 'bar' });
    });
