...
test('appendFbRoot', () => {
  expect(document.querySelector('div#fb-root'))
    .toBeFalsy();
  sagas.appendFbRoot();
  expect(document.querySelector('div#fb-root'))
    .toBeTruthy();
...
test('serviceAction', () => {
  ...
  expect(sagas.serviceAction('REQUEST', 'facebook')(action('REQUEST', 'facebook')))
    .toBe(true);
  expect(sagas.serviceAction('REQUEST', 'facebook')(action('PREPARE', 'facebook')))
    .toBe(false);
  expect(sagas.serviceAction('REQUEST', 'facebook')(action('REQUEST', 'google')))
    .toBe(false);
});
...
describe('loginFacebook', () => {
  ...
  it('calls success', () => {
    ...
    expect(generator.next().value)
      .toEqual(call(sagas.promises.fbGetMe, { fields: 'id,name' }));
...
describe('prepareFacebook', () => {
  ...
  it('calls success', () => {
    ...
    expect(generator.next().value)
      .toEqual(call(sagas.appendFbRoot));
    expect(generator.next().value)
      .toEqual(call(loadScript, '//connect.facebook.net/en_US/sdk.js'));
    expect(generator.next().value)
      .toEqual(call([window.FB, window.FB.init], {
        appId: 'test',
        version: 'v2.8',
        foo: 'bar',
      }));
...
test('watchSocialLoginFacebook', () => {
  ...
  expect(generator.next({ payload }).value)
    .toEqual(call(sagas.prepareFacebook, payload));
  generator.next();
  expect(generator.next({ payload }).value)
    .toEqual(call(sagas.loginFacebook, payload));
});
...
describe('loginGoogle', () => {
  ...
  it('calls success', () => {
    ...
    expect(generator.next().value)
      .toEqual(call(window.gapi.auth2.getAuthInstance));
    expect(generator.next(auth2).value)
      .toEqual(call([auth2, auth2.signIn], { scope: 'profile' }));
    expect(generator.next(user).value)
      .toEqual(call([user, user.getBasicProfile]));
    expect(generator.next(profile).value)
      .toEqual(call([profile, profile.getName]));
    expect(generator.next('name').value)
      .toEqual(call([profile, profile.getImageUrl]));
...
describe('prepareGoogle', () => {
  ...
  it('calls success', () => {
    ...
    expect(generator.next().value)
      .toEqual(call(sagas.promises.loadGoogleAuth2));
    expect(generator.next().value)
      .toEqual(call(window.gapi.auth2.init, {
        client_id: 'test',
        foo: 'bar',
      }));
  });
...
test('watchSocialLoginGoogle', () => {
  ...
  generator.next();
  expect(generator.next({ payload }).value)
    .toEqual(call(sagas.prepareGoogle, payload));
  generator.next();
  expect(generator.next({ payload }).value)
    .toEqual(call(sagas.loginGoogle, payload));
});

test('saga', () => {
  const generator = saga();
  expect(generator.next().value)
    .toEqual(fork(sagas.watchSocialLoginFacebook));
  expect(generator.next().value)
    .toEqual(fork(sagas.watchSocialLoginGoogle));
});
